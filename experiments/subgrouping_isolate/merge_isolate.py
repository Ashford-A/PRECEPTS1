...
def main():
    ...
    for out_fl in file_list:
        file_sets[int(out_fl.stem.split(
            "out__")[1].split("cv-")[1].split("_")[0])] |= {out_fl}
    ...
    mut_samps = {mut: mut.get_samples(use_mtree) for mut in muts_list}
    mut_genes = {mut: mut.get_labels()[0] for mut in muts_list}
    ...
    for cv_id, out_fls in file_sets.items():
        ...
        for ex_lbl in ['All', 'Iso', 'IsoShal']:
            ...
            assert sorted(out_preds.index) == sorted(muts_list), (
    ...
    for mut, pred_vals in pred_dfs['Iso'].iterrows():
        hld_samps = gene_samps[mut_genes[mut]] - mut_samps[mut]
        assert (pred_vals[hld_samps].apply(len) == 40).all(), (
            "Incorrect number of testing CV scores!")
    ...
    auc_dicts = {
        ex_lbl: {
            'all': pd.Series(dict(zip(muts_list, Parallel(
                n_jobs=12, prefer='threads', pre_dispatch=72)(
