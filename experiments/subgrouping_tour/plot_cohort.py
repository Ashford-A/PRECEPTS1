...
from HetMan.experiments.subvariant_tour import (
    type_file, metabric_dir, pnt_mtype)
...
def plot_classif_performance(auc_dfs, time_dfs, cdata, args):
    ...
    plt_times = sorted(
        [(clf, (time_df.loc[use_mtypes, 'avg'].apply(np.mean)
                + time_df.loc[use_mtypes, 'std'].apply(np.mean)).mean())
    ...
    sns.violinplot(x='Classif', y='AUC', data=auc_df, ax=ax, width=0.93,
                   palette=[choose_gene_colour(clf, clr_sat=0.7)
                            for clf, _ in plt_times],
                   order=[clf for clf, _ in plt_times], cut=0)

    ax.get_children()[0].set_alpha(0.61)
    ax.get_children()[2].set_alpha(0.61)

    ax.set_xticklabels(["{}\n({:.1f}s)".format(clf, clf_time)
                        for clf, clf_time in plt_times], size=25)
    ax.tick_params(axis='y', labelsize=23)

    ax.axhline(y=0.5, c='black', linewidth=2.9, linestyle=':', alpha=0.89)
    ax.set_xlabel('')
...
def main():
    ...
    out_use = pd.DataFrame([
        ...
        ]).groupby('Classif').filter(
            lambda outs: ('Exon__Location__Protein'
                          in set(outs.Levels))
            ).groupby(['Levels', 'Classif'])['Samps'].min()

    if out_use.shape[0] > 0:
    ...
    for (lvls, clf), ctf in out_use.iteritems():
        ...
        with bz2.BZ2File(os.path.join(
                base_dir, out_tag, "out-data__{}__{}.p.gz".format(lvls, clf)
                ), 'r') as f:
            time_dicts[clf][lvls] = pickle.load(f)['Time'].loc[:, 'Chrm']

        with bz2.BZ2File(os.path.join(
                base_dir, out_tag, "out-aucs__{}__{}.p.gz".format(lvls, clf)
                ), 'r') as f:
            auc_dicts[clf][lvls] = pickle.load(f).Chrm
