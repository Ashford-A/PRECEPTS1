...
to the arguments used by the pipeline. Each such subdirectory will contain
copies of the final output files, described below, as well as the following
folders:


## Final output files generated by the pipeline ##

Similarly to intermediate output files and `$TEMPDIR`, final output files are
stored in an automatically created directory called
`$DATADIR/dryads-research/subgrouping_test/`. This directory will contain
subdirectories of the form `$expr_source__$cohort__samps-$samp_cutoff` storing
experiment output across all mutation annotation combinations and
classification algorithms.

It will also contain the `plots` subdirectory, which will contain the plots
generated by the various `plot_*.py` scripts. For example,
`dryads-research.experiments.subgrouping_test.plot_cohort` will put the plots
it creates under `plots/cohort`; this behaviour is governed by the line
`plot_dir = os.path.join(base_dir, 'plots', 'cohort')` in `plot_cohort`.

Experiment output files are labelled
`<output-type>__$mut_levels__$classif.p.gz`, where `output-type` is the same
set of labels used for the copies of these files stored in `$TEMPDIR`. These
output types are as follows:
...
- `trnsf-preds.p.gz` Predictions made by trained models on cohorts other than
                     the one the model was trained on. This a counterpart to
                     `out-pred`; in this case the rows are subgroupings, the
                     columns are cohorts, and each entry of the matrix is a 
                     list of average predicted scores for the samples in the
                     cohort in the same order as they would be in `out-pheno`.

- `out-trnsf.p.gz` Assorted information on the results of transferring models
                   to other cohorts. This is a dictionary with an entry for
                   each cohort; for each cohort we have the samples used in
                   the cohort (useful for determining how cohorts such as
                   METABRIC_LumA differ from their parent cohorts), the binary
                   phenotype vector describing which samples in the transfer
                   cohort carry the subgrouping in question, and the transfer
                   AUCs calculated for the subgroupings with at least twenty
                   mutated samples in the transfer cohort.

See `gather_test.py` for how the preliminary versions of these output files are
generated during the course of running an experiment pipeline, and
`merge_test.py` for how these intermediate output files are consolidated into
their final output versions.
