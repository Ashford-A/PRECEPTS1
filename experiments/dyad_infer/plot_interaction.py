...
def get_mtype_clr(mtype):
    ...
    if sub_mtype == mtype_dict['Point']:
        mtype_clr = variant_clrs['Point']
    elif sub_mtype.cur_level in {'Form', 'Form_base', 'Exon', 'Protein'}:
        mtype_clr = variant_clrs['Point']

    elif not (mtype_dict['Loss'] & sub_mtype).is_empty():
...
def plot_mutual_similarity(use_mtypes, stat_dict, mutex_dict, siml_dict,
    ...
    plt.xticks(size=10)
    plt.yticks(size=10)
    ax.axhline(0, color='black', linewidth=0.9, linestyle='--', alpha=0.35)
    ax.axvline(0, color='black', linewidth=0.9, linestyle='--', alpha=0.35)
...
def plot_mutual_synergy(use_mtypes, stat_dict, mutex_dict, siml_dict, args):
    ...
    plt.yticks(size=10)
    ax.axhline(0, color='black', linewidth=0.9, linestyle='--', alpha=0.35)
    ax.axhline(1, color='black', linewidth=0.9, linestyle='--', alpha=0.35)
    ax.axvline(0, color='black', linewidth=0.9, linestyle='--', alpha=0.35)
...
def plot_similarity_symmetry(use_mtypes, stat_dict, siml_dict, args):
    ...
    ax.plot(plot_lim + [-1, 1], plot_lim + [-1, 1],
            linewidth=1.3, linestyle=':', color='#550000', alpha=0.31)
    ...
    plt.xlabel("Transcriptomic Similarity\nMutation 1 \u21d2 Mutation 2",
               size=17, weight='semibold')
