...
def main():
    ...
    sc_expr = load_SMMART_expr()[feat_list]
    ...
    out_smmart = {mtype: dict() for mtype in mtype_list}
    ...
    for i, mtype in enumerate(mtype_list):
        if (i % task_count) == args.task_id:
            ...
            out_smmart[mtype] = np.round(mut_clf.predict_omic(
    ...
    with open(os.path.join(args.use_dir, 'output',
        ...
        pickle.dump({'Pred': out_pred, 'Pars': out_pars, 'Time': out_time,
                     'Acc': out_acc, 'Coef': out_coef, 'SMMART': out_smmart,
