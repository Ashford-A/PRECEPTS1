...
def main():
    ...
    out_dfs = {k: [None for cv_id in range(40)]
               for k in ['Pred', 'Pars', 'Time', 'Acc', 'Coef', 'SMMART']}
    out_clf = None
    out_tune = None
    ...
    for cv_id, out_fls in file_sets.items():
        ...
        out_dfs['SMMART'][cv_id] = pd.DataFrame({
            mut: out_vals for out_dicts in out_list
            for mut, out_vals in out_dicts['SMMART'].items()
            }).transpose()

        for k in set(out_dfs.keys()) - {'Coef', 'SMMART'}:
        ...
        out_dfs['SMMART'][cv_id].columns = SMMART_expr.index
    ...
    smmart_df = pd.concat(out_dfs['SMMART'], axis=1)
    assert (pred_df.columns.value_counts() == 10).all(), (
        "Inconsistent number of CV scores across cohort samples!")
    ...
    for out_df in [pred_df, pars_df, time_df, acc_df, coef_df, smmart_df]:
    ...
    smmart_df = pd.DataFrame({
        mtype: smmart_df.loc[mtype].groupby(level=0).apply(lambda x: x.values)
        for mtype in use_muts
        }).transpose()
    ...
    with bz2.BZ2File(os.path.join(args.use_dir, 'merge',
                                  "out-smmart{}.p.gz".format(out_tag)),
                     'w') as fl:
        pickle.dump(smmart_df, fl, protocol=-1)
