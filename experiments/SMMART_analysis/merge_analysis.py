...
def main():
    ...
    for smmart_file in Path(args.use_dir, 'merge').glob("out-smmart_*.p.gz"):
        with bz2.BZ2File(smmart_file, 'r') as fl:
            smmart_data = pickle.load(fl)
        smmart_df = smmart_df.append(smmart_data)

    assert sorted(muts_list) == sorted(smmart_df.index), (
        "Tested mutations missing from single-cell predictions!")
    with bz2.BZ2File(os.path.join(args.use_dir, "out-smmart.p.gz"), 'w') as fl:
