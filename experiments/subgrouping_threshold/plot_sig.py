...
plot_dir = os.path.join(base_dir, 'plots', 'sig')


def plot_sub_comparison(orig_aucs, auc_df, orig_phenos, pheno_dict, args):
    fig, ax = plt.subplots(figsize=(13, 8))

    cur_gene = {get_label(mtype) for mtype in orig_aucs.index}
    ...
    base_aucs = np.array(orig_aucs.loc[base_mtype, 'CV'])

    ax.scatter(base_size, orig_aucs['mean'][base_mtype], c='none',
               edgecolors=variant_clrs['Point'], s=401, alpha=0.53,
               linewidths=3.7)
    plt_min = orig_aucs['mean'][base_mtype] - 0.13

    orig_df = pd.DataFrame({mtype: {'Size': orig_phenos[mtype].sum()}
                            for mtype in orig_aucs.index}).transpose()
    orig_df['AUC'] = orig_aucs['mean'][orig_df.index]
    ...
    orig_lbl = "{} subgroupings".format(cur_gene)
    if (np.array(orig_aucs.loc[best_orig, 'CV']) > base_aucs).all():
        orig_lbl += "*"

    lgnd_ptchs = [Patch(color=variant_clrs['Point'],
                        alpha=0.43, label=orig_lbl)]

    for annt_lbl in ['PolyPhen', 'SIFT']:
        annt_mtypes = {mtype for mtype in auc_df.index
        ...
        if annt_mtypes:
            ...
            annt_df['AUC'] = auc_df['mean'][annt_df.index]
            ...
            annt_lbl = "{} thresholds".format(annt_lbl)
            if (np.array(auc_df.loc[best_annt, 'CV']) > base_aucs).all():
                annt_lbl += "*"

            lgnd_ptchs += [Patch(color=annt_clr, alpha=0.43, label=annt_lbl)]
    ...
    ax.text(base_size, orig_aucs['mean'][base_mtype] + (1 - plt_min) / 31,
...
def main():
    ...
    orig_datas = [
        out_file.parts[-2:] for out_file in Path(orig_dir).glob(
            os.path.join("{}__{}__samps-*".format(use_src, args.cohort),
    ...
    orig_aucs = pd.concat(auc_dict.values())
    ...
    orig_aucs = orig_aucs.loc[orig_mtypes]
    os.makedirs(os.path.join(plot_dir, args.cohort), exist_ok=True)

    for gene, auc_vec in orig_aucs.groupby(get_label):
        use_aucs = auc_vec.loc[[mtype for mtype in auc_vec.index
                                if not isinstance(mtype, RandomType)]]

        if len(use_aucs) > 1:
            thresh_mtypes = [
                mtype for mtype in auc_df.index
                if (mtype.annot in {'PolyPhen', 'SIFT'}
                    and get_label(mtype.base_mtype) == gene
                    and (get_subtype(mtype.base_mtype)
                         & copy_mtype).is_empty())
                ]

            if thresh_mtypes:
                plot_sub_comparison(use_aucs, auc_df.loc[thresh_mtypes],
