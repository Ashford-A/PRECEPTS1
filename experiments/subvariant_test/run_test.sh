...
OUTDIR=$TEMPDIR/HetMan/subvariant_test/$expr_source/$out_tag/$mut_levels/$classif
FINALDIR=$DATADIR/HetMan/subvariant_test/${expr_source}__$out_tag

export RUNDIR=$CODEDIR/HetMan/experiments/subvariant_test
...
mkdir -p $FINALDIR $TEMPDIR/HetMan/subvariant_test/$expr_source/setup
...
dvc run -d $firehose_dir -d $mc3_file -d $gencode_file -d $subtype_file \
	-d $RUNDIR/setup_test.py -d $CODEDIR/HetMan/environment.yml \
	-o setup/muts-list.p -m setup/muts-count.txt \
	-f setup.dvc --overwrite-dvcfile python $RUNDIR/setup_test.py \
